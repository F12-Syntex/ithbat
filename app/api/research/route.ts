...
import { type Source, type ResearchStep } from "@/types/research";

async function fetchQuranVerse(
  surah: number,
  ayah: number,
): Promise<{ arabic: string; surahName: string } | null> {
  try {
...
function extractQuranRefsFromFormatted(
  text: string,
): Array<{ surah: number; ayah: number; fullMatch: string }> {
  const refs: Array<{ surah: number; ayah: number; fullMatch: string }> = [];
  const seen = new Set<string>();

  // Match [Quran X:Y](url) patterns
  const linkPattern =
    /\[Quran\s+(\d{1,3}):(\d{1,3})(?:-\d+)?\]\(https?:\/\/quran\.com\/\d+\/\d+\)/gi;
  let match;
...
async function enrichQuranVerses(text: string): Promise<string> {
  ...
  for (const { surah, ayah, fullMatch, verse } of results) {
    ...
    if (!fullMatch.startsWith("[")) {
      enriched = enriched.replace(fullMatch, `[${fullMatch}](${linkUrl})`);
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  const STEP_TITLES: Record<string, Record<string, string>> = {
    en: {
      ...
      understandingFollowUp: "Understanding your follow-up",
      searching: "Searching Islamic sources",
      formatting: "Formatting response",
      translating: `Translating to ${languageNativeName}`,
    },
    ar: {
      understanding: "فهم سؤالك",
      understandingFollowUp: "فهم متابعتك",
      searching: "البحث في المصادر الإسلامية",
      formatting: "تنسيق الإجابة",
      translating: `الترجمة إلى ${languageNativeName}`,
    },
    ja: {
      understanding: "質問を理解中",
      understandingFollowUp: "フォローアップを理解中",
      searching: "イスラーム文献を検索中",
      formatting: "回答をフォーマット中",
      translating: `${languageNativeName}に翻訳中`,
    },
    ur: {
      understanding: "آپ کا سوال سمجھ رہے ہیں",
      understandingFollowUp: "آپ کا فالو اپ سمجھ رہے ہیں",
      searching: "اسلامی ذرائع تلاش کر رہے ہیں",
      formatting: "جواب فارمیٹ کر رہے ہیں",
      translating: `${languageNativeName} میں ترجمہ کر رہے ہیں`,
    },
    fr: {
      understanding: "Compréhension de votre question",
      understandingFollowUp: "Compréhension de votre suivi",
      searching: "Recherche des sources islamiques",
      formatting: "Mise en forme de la réponse",
      translating: `Traduction en ${languageNativeName}`,
    },
    zh: {
      understanding: "理解您的问题",
      understandingFollowUp: "理解您的后续问题",
      searching: "搜索伊斯兰文献",
      formatting: "格式化回答",
      translating: `正在翻译为${languageNativeName}`,
    },
  };
  const stepTitle = (key: string) =>
    STEP_TITLES[language]?.[key] ?? STEP_TITLES.en[key] ?? key;
      ...
      try {
        ...
        send({
          ...
          content:
            ...
            )[language] || "Searching the web for Islamic evidence...\n",
        });
        ...
        const foundMsg =
          ...
          )[language] || `Found ${foundUrls.length} source links\n`;
        ...
        formattedResponse = await enrichTermDefinitions(
          formattedResponse,
          client,
        );
